...
import { baseUIMultiCombobox } from '#panda/recipes'
import preview from '#sb/preview.tsx'
import { utils } from '#yamatomo/panda-preset'
import { baseUIMultiCombobox as config } from './baseUIMultiCombobox.ts'

const sizes = utils.keys(config.variants.size)
const variants = utils.keys(config.variants.variant)

type Args = { styles: ReturnType<typeof baseUIMultiCombobox> } & Partial<
  {
  ...
  return (
    <Combobox.Root items={items} multiple>
      <Field.Root {...{ disabled, invalid }} className={cx(styles.root, css({ minW: '64' }))}>
        <Field.Control
          render={(innerProps) => (
            <Combobox.Chips {...innerProps} className={styles.chips} ref={containerRef}>
              <Combobox.Value>
                {(value: typeof defaultItems) => (
                  <>
                    {value.map((language) => (
                      <Combobox.Chip
                        key={language.value}
                      ...
                      >
                        ...
                        <Combobox.ChipRemove
                          ...
                          aria-label="Remove"
                          disabled={
                            'disabled' in innerProps ? Boolean(innerProps.disabled) : undefined
                          }
                        >
                    ...
                    <Combobox.Input
                      ...
                      placeholder={value.length > 0 ? '' : 'e.g. item 1'}
...
const meta = preview.meta({
  args: { disabled: false, invalid: false, styles: baseUIMultiCombobox() },
  argTypes: {
...
export const Variants = meta.story({
  parameters: {
    tableDecorator: {
      cols: variants,
      rows: [{ items: variants.map((variant) => ({ styles: baseUIMultiCombobox({ variant }) })) }],
    },
  },
})
...
export const Size = meta.story({
  parameters: {
    tableDecorator: {
      cols: sizes,
      rows: [{ items: sizes.map((size) => ({ styles: baseUIMultiCombobox({ size }) })) }],
    },
  },
})
