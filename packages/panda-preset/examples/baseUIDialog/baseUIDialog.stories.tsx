...
import { baseUIDialog, button } from '#panda/recipes'
import preview from '#sb/preview.tsx'
import { utils } from '#yamatomo/panda-preset'
import { baseUIDialog as config } from './baseUIDialog.ts'

const sizes = utils.keys(config.variants.size)
const placements = utils.keys(config.variants.placement)
const stacking = utils.keys(config.variants.stacking)
const motionPresets = utils.keys(config.variants.motionPreset)
...
const Component = ({ size, placement, stacking, motionPreset, children }: Args) => {
  const { backdrop, body, popup, header, title, closeTrigger } = baseUIDialog({
    motionPreset,
    placement,
    size,
    stacking,
