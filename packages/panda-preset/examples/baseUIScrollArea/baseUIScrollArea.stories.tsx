...
import { baseUIScrollArea } from '#panda/recipes'
import preview from '#sb/preview.tsx'
import { utils } from '#yamatomo/panda-preset'
import { baseUIScrollArea as config } from './baseUIScrollArea.ts'

const sizes = utils.keys(config.variants.size)
const variants = utils.keys(config.variants.variant)

type Args = ReturnType<typeof baseUIScrollArea> & {
  containerClassName?: string
  listClassName?: string
}

const meta = preview.meta({
  args: { ...baseUIScrollArea(), containerClassName: css({ h: '[50vh]', w: '[700px]' }) },
  argTypes: {
...
export const Variants = meta.story({
  ...
  parameters: {
    tableDecorator: {
      ...
      rows: [
        {
          items: variants.map((variant) => baseUIScrollArea({ variant })),
        },
      ],
    },
  },
...
export const Sizes = meta.story({
  ...
  parameters: {
    tableDecorator: {
      ...
      rows: [
        {
          items: sizes.map((size) => baseUIScrollArea({ size, variant: 'always' })),
        },
      ],
    },
  },
...
const shadowStyles = baseUIScrollArea({ hasShadow: true })
